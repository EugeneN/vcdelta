...
(function() {
  var BUILD_ID_FN, BUNDLE_FILTER, DELTA_FN, E_BAD_ARGS, E_ERROR, E_OK, E_VERSION, VERSION, apply_delta, calc_delta, from_json, fs, gen_delta, get_build_id, main, minimist, mkdirp, parse_args, path, read, say, toDict, to_json, to_path, vcd, vcdiff, write,
    __slice = [].slice;
  ...
  gen_delta = function(base, target) {
    return [path.basename(target), calc_delta(read(base), read(target))];
  };
  ...
  main = function(argv) {
    var delta_root, deltas, new_build_root, old_build_id, old_build_root, realms, v, version, _ref;
    ...
    return deltas.map(function(_arg) {
      ...
      return write(to_path(delta_root, old_build_id, realm, DELTA_FN), to_json(delta));
    });
  };
